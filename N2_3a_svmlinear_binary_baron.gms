...
*Dataset        :   Binary
*ML Classifier  :   SVM Linear kernel : 3a
*Solver         :   BARON
*Threads        :   1
*Time limit     :   3600s
*==============================================================================
...
$gdxin 3_svmlinear_binary_gdx.gdx
$load input_offset=input_offset
$gdxin

*1/sd
input_gain(f)  'vector of input offsets (mean)'
$gdxin 3_svmlinear_binary_gdx.gdx
...
eq_svm1..  df =e= sum(f, w_sv(f)*inp(f)) + bias;
...
df.l = sum(f, w_sv(f)*inp.l(f)) + bias;
*********************************************************
MODEL N2 / ALL/;
N2.optfile=0;
option nlp=conopt3;
